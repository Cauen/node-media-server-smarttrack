...
// const serverip = "http://138.197.71.168:8080";
const serverip = "http://nead.tk:8080";
const rtmpAddress = "rtmp://smarttrack.ga:1935/app/";
let token = false;
let rtcamheight = 0;
let rtcamwidth = 0;
...
class NodeRtmpSession {
    ...
    switch (dataMessage.cmd) {
      case "@setDataFrame":
        if (dataMessage.dataObj) {
          ...
          rtcamheight = dataMessage.dataObj.height;
          rtcamwidth = dataMessage.dataObj.width;
  ...
  onReceiveAudio(invokeMessage) {
    ...
    Logger.log(
      `[rtmp play] receiveAudio=${this.isReceiveAudio} id=${this.id} `
    );
  }
  ...
  onReceiveVideo(invokeMessage) {
    ...
    Logger.log(
      `[rtmp play] receiveVideo=${this.isReceiveVideo} id=${this.id} `
    );
  }
